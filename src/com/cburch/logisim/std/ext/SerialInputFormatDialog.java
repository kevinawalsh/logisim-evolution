...
public class SerialInputFormatDialog extends JDialog implements JInputDialog<SerialInputFormat> {
  ...
  private JPanel delimiters = new JPanel(new FlowLayout(FlowLayout.LEFT, 2, 2));
  ...
  private final JRadioButton rawFormatButton = new JRadioButton("raw bytes");
  private final JRadioButton parsedFormatButton = new JRadioButton("delimited text");

  // Delimiters subsection
  private final String[] delPrebaked = {
    "lines", "[\\n\\r]",
    "whitespace", "[ \\t\\n\\r]",
    "comma-separated", "[, \\t\\n\\r]",
    "punctuation", "[, :()[]{}<>;\\t\\n\\r]"
  };
  private final JRadioButton[] delPrebakedButtons;
  private final JRadioButton delCustomButton = new JRadioButton("custom delimiters:");
  private final JTextField delCustomTextfield = new JTextField(24);
  ...
  private SerialInputFormat format, oldFormat;

  public boolean isConfirmed() { return confirmed; }

  public SerialInputFormat getValue() { return confirmed ? format : oldFormat; }

  public SerialInputFormatDialog(Window owner, SerialInputFormat format) {
    super(owner, "Serial Data Format", ModalityType.APPLICATION_MODAL);
    setDefaultCloseOperation(DISPOSE_ON_CLOSE);

    this.format = format;

    // fonts
    delCustomTextfield.setFont(monoPlain);
    ...
    col.add(top);
    for (JPanel p : new JPanel[]{ illustration, delimiters, demonstration }) {
    ...
    col.add(buttonWithDescription(rawFormatButton,
          "Each byte arriving from the serial port is returned directly."));
    col.add(Box.createVerticalStrut(10));
    col.add(buttonWithDescription(parsedFormatButton,
          "Data arriving from the serial port is split into records, each containing one or more values."));
  ...
  public void setValue(SerialInputFormat newFormat) {
    ...
    if (format.isRaw()) {
      delCustomTextfield.setText(delPrebaked[1]);
      fmtCustomTextfield.setText(fmtPrebaked[1]);
      rawFormatButton.doClick();
    } else {
      String del = format.getDelimiterString();
      String fmt = format.getFormatString();
      delCustomTextfield.setText(del);
  ...
  private void illustrate() {
    ...
    delimiters.invalidate();
    delimiters.revalidate();
    delimiters.repaint();
    demonstrate();
  }

  private void illustrateRaw() {
    format = new SerialInputFormat();
    illustration.add(box("Serial data looks like "));
    illustration.add(box("byte", 0x003300, 0xccffcc, false));
    illustration.add(box("byte", 0x003300, 0xccffcc, false));
    illustration.add(box("byte", 0x003300, 0xccffcc, false));
    illustration.add(box("byte", 0x003300, 0xccffcc, false));
    illustration.add(box(" ..."));
    delimiters.add(box("with nothing between bytes."));
  }

  private void illustrateParsed() {
    String delText = delCustomTextfield.getText();
    String fmtText = fmtCustomTextfield.getText();
    format = new SerialInputFormat(delText, fmtText);
    if (!format.isValid()) {
      String msg = "Error: " + format.errorMessage();
      illustration.add(box(msg, 0x800000, 0xFFE6E6, false));
      delimiters.add(box(" "));
    } else {
      illustration.add(box("Each record is "));
      for (SerialInputFormat.Token t : format.getTokens()) {
        if (t instanceof SerialInputFormat.StaticToken) {
          SerialInputFormat.StaticToken tt = (SerialInputFormat.StaticToken)t;
          addTextAsBoxes(illustration, tt.bytes);
        } else if (t instanceof SerialInputFormat.ByteToken) {
          illustration.add(box("byte", 0x003300, 0xccffcc, false));
        } else if (t instanceof SerialInputFormat.SignedDecimalToken) {
          illustration.add(box(t.width+"-bit int", 0x00264d, 0xe6f2ff, false));
        } else if (t instanceof SerialInputFormat.RadixToken) {
          SerialInputFormat.RadixToken tt = (SerialInputFormat.RadixToken)t;
          if (tt.radix == 16)
      ...
      byte[] delims = format.getDelims();
      if (delims == null) {
        delimiters.add(box("with nothing between each record."));
      } else {
        delimiters.add(box("followed by: "));
        for (int i = 0; i < delims.length; i++) {
          if (i > 0 && i == delims.length - 1)
            delimiters.add(box(" or "));
          else if (i > 0)
            delimiters.add(box(", "));
          byte b = delims[i];
          if (b == '\n')
            delimiters.add(box("newline", 0x003300, 0xccffcc, false));
          else if (b == '\r')
            delimiters.add(box("carriage-return", 0x003300, 0xccffcc, false));
          else if (b == '\t')
            delimiters.add(box("tab", 0x003300, 0xccffcc, false));
          else if (b == ' ')
            delimiters.add(box("space", 0x003300, 0xccffcc, false));
          else if (0x20 < b && b <= 0x7E)
            delimiters.add(box(""+(char)b, 0x000000, 0xffffff, true));
          else 
            delimiters.add(box(String.format("\\x%02x", b), 0x003300, 0xccffcc, true));
        }
      }
    }
  }
  ...
  private void demonstrate() {
    ...
    if (!format.isValid()) {
      ...
      byte[] delims = format.getDelims();
      if (delims != null) {
        byte b = delims[random.nextInt(delims.length)];
        if (b == '\n')
          demonstration.add(box("\\n", 0x003300, 0xccffcc, false));
        else if (b == '\r')
          demonstration.add(box("\\r", 0x003300, 0xccffcc, false));
        else if (b == '\t')
          demonstration.add(box("     ", 0x003300, 0xccffcc, false));
        else if (b == ' ')
          demonstration.add(box(" ", 0x003300, 0xccffcc, false));
        else if (0x20 < b && b <= 0x7E)
          demonstration.add(box(""+(char)b, 0x000000, 0xffffff, true));
        else 
          demonstration.add(box(String.format("\\x%02x", b), 0x003300, 0xccffcc, true));
      }
    }
  ...
  public static void main(String[] args) {
    SwingUtilities.invokeLater(() -> {
      SerialInputFormatDialog d = new SerialInputFormatDialog(null, new SerialInputFormat("[\\n]", "%3d, %6d"));
      d.setVisible(true);
      System.exit(0);
    });
  }
